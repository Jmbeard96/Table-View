...
class HeroTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let heroCell = cell as? HeroTableViewCell {
            if let data = heroData {
                let hero = data[indexPath.row]
                heroCell.HeroName.text = hero.name
                if let imageName = hero.imageName {
                    heroCell.HeroImage?.image = UIImage(named: imageName)
                }
                else{
                    heroCell.HeroImage.image = nil
                }
            }
        }
